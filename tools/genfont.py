# genfont.py — generates src/font_rom.svh from the embedded 8x8 font
...
#
# test code imports FONT from here so RTL and checks share one table.
...
def render():
    """The .svh text, returned rather than written.

    Split out from main() on 2026-08-08 so test/test_font.py can check that
    the COMMITTED font_rom.svh is still what this generator produces. A
    generated file that is committed can drift from its generator in either
    direction -- hand-edited, or left stale after a table change -- and
    neither shows up anywhere until the screen renders the wrong glyph.
    """
    lines = [
    ...
    for code, rows in sorted(FONT.items()):
        ...
        for i, b in enumerate(rows):
            g |= b << (8 * i)
        lines.append(f"        8'h{code:02X}: g = 64'h{g:016X};")
    lines += [
        "        default: g = 64'd0;",
        "    endcase",
        "    font_row = g[fr*8 +: 8];",
...
def main():
    out = Path(__file__).parent.parent / "src" / "font_rom.svh"
    # encoding= is NOT optional. Path.write_text() uses the LOCALE encoding,
    # which on this Windows host is cp1252 and on CI is UTF-8 -- so the same
    # generator produced a different FILE depending on who ran it, and the
    # committed one has been cp1252 (an em-dash as 0x97) since it was made.
    # Nothing noticed because iverilog and yosys read it as bytes; a test that
    # compares the file to the generator notices immediately.
    out.write_text(render(), encoding="utf-8")
    print(f"wrote {out} ({len(FONT)} glyphs)")
